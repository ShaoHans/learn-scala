...
object S02_receiver_custom {
    def main(args: Array[String]): Unit = {
        // [2]可并行跑2个任务
        // 1个给receiverjob的task，
        // 另一个给batch计算的job（只不过如果batch比较大，你期望n>2,因为多出来的线程可以跑并行的batch@job@task）
        val conf = new SparkConf().setAppName("S02_receiver_custom").setMaster("local[2]")
        ...
        val receiver = context.receiverStream(new MyReceiver("172.28.93.145",8888))
        receiver.print()
